...
dhallToNix e = loop (Dhall.Core.normalize e)
  where
    ...
    loop (Pi _ _ _) = return (Fix (NSet NNonRecursive []))
    ...
    loop Bool = return (Fix (NSet NNonRecursive []))
    ...
    loop Natural = return (Fix (NSet NNonRecursive []))
    ...
    loop NaturalBuild = do
        ...
        let e1 = Fix (NBinary NApp (Fix (NBinary NApp "k" (Fix (NSet NNonRecursive [])))) (Fix (NAbs "n" e0)))
    ...
    loop Integer = return (Fix (NSet NNonRecursive []))
    ...
    loop IntegerToDouble = do
        return (Fix (NAbs "x" "x"))
    loop Double = return (Fix (NSet NNonRecursive []))
    loop (DoubleLit (DhallDouble n)) = return (Fix (NConstant (NFloat (realToFrac n))))
    loop DoubleShow = do
        return "toString"
    loop Text = return (Fix (NSet NNonRecursive []))
    ...
    loop TextShow = do
        ...
        return ("t" ==> quoted)
    loop List = return (Fix (NAbs "t" (Fix (NSet NNonRecursive []))))
    ...
    loop (ListLit _ bs) = do
        ...
        return (Fix (NList bs'))
    loop ListBuild = do
        let e0 = Fix (NBinary NApp "k" (Fix (NSet NNonRecursive [])))
    ...
    loop Optional = return (Fix (NAbs "t" (Fix (NSet NNonRecursive []))))
    loop (Some a) = loop a
    ...
    loop (Record _) = return (Fix (NSet NNonRecursive []))
    loop (RecordLit a) = do
        a' <- traverse loop a
        let a'' = do
    ...
    loop (Union _) = return (Fix (NSet NNonRecursive []))
    ...
    loop (CombineTypes _ _) = return (Fix (NSet NNonRecursive []))
    loop (Merge a b _) = do
        a' <- loop a
        b' <- loop b
